...
class Load(Base):
    ...
    equipment_type = Column(String(50), nullable=True)  # Dry Van, Reefer, Flatbed, etc.
    weight = Column(Numeric(12, 2), nullable=True)  # lbs or kg
...
class ShipperStop(Base):
    ...
    type = Column(String(50), nullable=True)  # live_load, etc
    pickup_date = Column(Date, nullable=True)
    time_start = Column(Time, nullable=True)
    time_end = Column(Time, nullable=True)
    pallet_info = Column(JSONB, nullable=True)  # temperature, weight, dims, etc
...
class CarrierSegment(Base):
    ...
    rating = Column(Integer, nullable=True)  # 1-5 carrier performance
    on_time = Column(Boolean, nullable=True)  # delivered on time
